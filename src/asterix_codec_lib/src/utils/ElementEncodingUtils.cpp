...
void ElementEncodingUtils::checkValidityRange(string *elementName, double elementValue, Range *elementRange,
    ...
    if (isInsideRange == false) {
        ...
        string message =
                "Out of range Element value '" + (*elementName) + "': it is " + to_string(elementValue) +
        ...
        failureReport->addFailure(categoryNumber, 0, (*elementName), nullptr, message,
                                  ReturnCodes::OUT_OF_RANGE_ENCODED_ELEMENT);
    }
}

unsigned char *
ElementEncodingUtils::encodeDecimal(ElementValue *element, Range *validityRange, int lengthInBit, double multFactor,
                                    string *elementName,
                                    unsigned int categoryNumber) {
    int decodedValue = 0;
...
unsigned char *
ElementEncodingUtils::encodeDecimalToCA2(ElementValue *element, Range *validityRange, int lengthInBit,
                                         double multFactor, string *elementName,
                                         unsigned int categoryNumber) {
...
unsigned char *ElementEncodingUtils::encodeTargetId(ElementValue *element, int lengthInBit) {
    ...
    if (StringValue *stringValue = dynamic_cast<StringValue *>(element)) {
        ...
        for (int i = 0; i < noTargetIdChars; i++) {
            char charToSearch = originalValue.at(
                    i);    // Here we extract the CHAR to be searched inside the ICAO Alphabet vector
            auto index = distance(icaoAlphabet->begin(),
                                  find(icaoAlphabet->begin(), icaoAlphabet->end(), charToSearch));
