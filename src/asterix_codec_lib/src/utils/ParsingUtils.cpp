...
unsigned long long ParsingUtils::parseDatablock(unsigned char *ASTERIX_datablock, unsigned short int datablockCategory,
                                                int datablockLength,
    ...
    recordNumber += parseAllRecords(ASTERIX_datablock, bufferIndex, datablockCategory, datablockLength,
                                    categoriesDescription, threadId);
    return recordNumber;
}

int ParsingUtils::parseAllRecords(unsigned char *ASTERIX_datablock, int bufferIndex, int datablockCategory,
                                  int datablockLength,
                                  unordered_map<int, CategoryDescription *> *categoriesDescription,
                                  pthread_t threadId) {
    ...
    while (bytesRead < datablockLength) {
        ...
        sacSicKey =
                datablockCategory * 1000 * 1000;    // CAT number move to left to fit SAC (3 digits) and SIC (3 digits)
        ...
        if (categoryDescription_it == categoriesDescription->end() || FRN_listSize == 0) {
            ...
            failureReport->addFailure(datablockCategory, bufferIndex + bytesRead, "", datablockToReport,
                                      "Unknown Category number, Datablock skipped!",
        ...
        for (unsigned short mandatoryFRN_toCheck : *(categoryDescription->getMandatoryItemFRNs())) {
            if (find(FRN_list.begin(), FRN_list.end(), mandatoryFRN_toCheck) == FRN_list.end()) {
                ...
                failureReport->addFailure(datablockCategory, bufferIndex + bytesRead,
                                          itemsDescription->at(mandatoryFRN_toCheck).getName(),
                                          entireDatablock,
                                          "Mandatory Item for the current Category was missing, Datablock skipped!",
        ...
        for (int i = 0; i < FRN_listSize; i++) {
            ...
            if (FRN > itemsDescription->size()) {
                ...
                unsigned char *entireDatablock = new unsigned char[datablockLength]();
                memcpy(entireDatablock, (ASTERIX_datablock + bufferIndex), datablockLength);
                failureReport->addFailure(datablockCategory, bufferIndex + bytesRead, "", entireDatablock,
                                          "Unknown FRN, Datablock skipped!",
            ...
            switch (itemType) {
                case ItemTypeEnum::FixedLength:
                    itemLengthInByte = parseFixedLengthItem(ASTERIX_datablock, itemToParse, bufferIndex + bytesRead,
                                                            itemToParse->isItemToBeDecoded(),
                                                            objectPool);

                    break;

                case ItemTypeEnum::ExtendedLength:
                    itemLengthInByte = parseExtendedLengthItem(ASTERIX_datablock, itemToParse, bufferIndex + bytesRead,
                                                               itemToParse->isItemToBeDecoded(),
                                                               objectPool);
                    break;

                case ItemTypeEnum::CompoundLength:
                    itemLengthInByte = parseCompoundLengthItem(ASTERIX_datablock, itemToParse, bufferIndex + bytesRead,
                                                               itemToParse->isItemToBeDecoded(),
                                                               compoundSubItemsDescription, datablockCategory,
                                                               objectPool);
                    break;

                case ItemTypeEnum::RepetitiveLength:
                    itemLengthInByte = parseRepetitiveLengthItem(ASTERIX_datablock, itemToParse,
                ...
                case ItemTypeEnum::ExplicitLength:
                    itemLengthInByte = parseExplicitLengthItem(ASTERIX_datablock, itemToParse, bufferIndex + bytesRead,
                                                               itemToParse->isItemToBeDecoded(),
                                                               refItemDescription, compoundSubItemsDescription,
                                                               spfItemDescription, datablockCategory,
                                                               objectPool);
                    break;
                ...
                default: {
                    ...
                    memcpy(datablockToReport, (ASTERIX_datablock + bufferIndex), datablockLength);
                    failureReport->addFailure(datablockCategory, bufferIndex + bytesRead, itemToParse->getName(),
                                              datablockToReport,
                                              "Unknown Item type!",
...
int
ParsingUtils::parseFixedLengthItem(unsigned char *ASTERIX_datablock, Item *itemToParse, int index,
                                   bool isItemToBeDecoded, ParsedRecord *objectPool) {
...
int ParsingUtils::parseExtendedLengthItem(unsigned char *ASTERIX_datablock, Item *itemToParse, int index,
...
int ParsingUtils::parseRepetitiveLengthItem(unsigned char *ASTERIX_datablock, Item *itemToParse, int index,
                                            bool isItemToBeDecoded,
                                            ParsedRecord *objectPool) {
    // Variables definition
...
int ParsingUtils::parseCompoundLengthItem(unsigned char *ASTERIX_datablock, Item *itemToParse, int index,
                                          bool isItemToBeDecoded,
    ...
    for (unsigned int j = 0; j < itemsIndicator.size(); j++) {
        ...
        switch (subitemType) {
            case ItemTypeEnum::FixedLength:
                itemLength = parseFixedLengthItem(ASTERIX_datablock, subitem, index + bytesRead, isItemToBeDecoded,
                                                  objectPool);

                break;

            case ItemTypeEnum::ExtendedLength:
                itemLength = parseExtendedLengthItem(ASTERIX_datablock, subitem, index + bytesRead, isItemToBeDecoded,
                                                     objectPool);

                break;

            case ItemTypeEnum::RepetitiveLength:
                itemLength = parseRepetitiveLengthItem(ASTERIX_datablock, subitem, index + bytesRead, isItemToBeDecoded,
                                                       objectPool);
                break;

            default: {
                // Unknown Item type, failure added to the report variable
                failureReport->addFailure(categoryNumber, index + bytesRead, subitem->getName(), nullptr,
                                          "Unknown Compound subitem type!",
                                          ReturnCodes::PARSING_ERROR);
...
int ParsingUtils::parseExplicitLengthItem(unsigned char *ASTERIX_datablock, Item *itemToParse, int index,
                                          bool isItemToBeDecoded,
    ...
    if (itemToParse->getName() == kREF) {
        ...
        if (facadeCommonVariables->isRefDecodingEnabled() && refItemDescription->size() > 0) {
            ...
            for (unsigned int i = 0; i < itemsIndicator.size(); i++) {
                ...
                switch (REF_itemType) {
                    case ItemTypeEnum::FixedLength:
                        bytesRead += parseFixedLengthItem(ASTERIX_datablock, REF_item, index + bytesRead,
                                                          REF_item->isItemToBeDecoded(), objectPool);

                        break;

                    case ItemTypeEnum::ExtendedLength:
                        bytesRead += parseExtendedLengthItem(ASTERIX_datablock, REF_item, index + bytesRead,
                                                             REF_item->isItemToBeDecoded(),
                                                             objectPool);

                        break;

                    case ItemTypeEnum::RepetitiveLength:
                        bytesRead += parseRepetitiveLengthItem(ASTERIX_datablock, REF_item, index + bytesRead,
                                                               REF_item->isItemToBeDecoded(),
                                                               objectPool);
                        break;

                    case ItemTypeEnum::CompoundLength:
                        bytesRead += parseCompoundLengthItem(ASTERIX_datablock, REF_item, index + bytesRead,
                                                             REF_item->isItemToBeDecoded(),
                                                             subItemsDescription,
                    ...
                    case ItemTypeEnum::ExplicitLength:
                        bytesRead += parseExplicitLengthItem(ASTERIX_datablock, REF_item, index + bytesRead,
                                                             REF_item->isItemToBeDecoded(),
                                                             refItemDescription,
                                                             subItemsDescription, spfItemDescription, categoryNumber,
                                                             objectPool);

                        break;
                    default:
                        // Unknown Item type, failure added to the report variable
                        failureReport->addFailure(categoryNumber, index + bytesRead, REF_item->getName(), nullptr,
                                                  "Unknown REF Item type!",
                                                  ReturnCodes::PARSING_ERROR);
        ...
        if (facadeCommonVariables->isSpfDecodingEnabled() && spfItemDescription->size() > 0) {
            ...
            for (unsigned int i = 0; i < elementsDescriptionSize; i++) {
                ...
                switch (SPF_itemType) {
                    case ItemTypeEnum::FixedLength:
                        bytesRead += parseFixedLengthItem(ASTERIX_datablock, SPF_item, index + bytesRead,
                                                          SPF_item->isItemToBeDecoded(), objectPool);
                        break;
                    case ItemTypeEnum::ExtendedLength:
                        bytesRead += parseExtendedLengthItem(ASTERIX_datablock, SPF_item, index + bytesRead,
                                                             SPF_item->isItemToBeDecoded(),
                                                             objectPool);
                        break;
                    case ItemTypeEnum::CompoundLength:
                        bytesRead += parseCompoundLengthItem(ASTERIX_datablock, SPF_item, index + bytesRead,
                                                             SPF_item->isItemToBeDecoded(),
                                                             subItemsDescription,
                                                             categoryNumber, objectPool);
                        break;
                    case ItemTypeEnum::RepetitiveLength:
                        bytesRead += parseRepetitiveLengthItem(ASTERIX_datablock, SPF_item, index + bytesRead,
                                                               SPF_item->isItemToBeDecoded(),
                                                               objectPool);
                        break;
                    case ItemTypeEnum::ExplicitLength:
                        bytesRead += parseExplicitLengthItem(ASTERIX_datablock, SPF_item, index + bytesRead,
                                                             SPF_item->isItemToBeDecoded(),
                                                             refItemDescription,
                                                             subItemsDescription, spfItemDescription, categoryNumber,
                                                             objectPool);
                    ...
                    default:
                        // Unknown Item type, failure added to the report variable
                        failureReport->addFailure(categoryNumber, index + bytesRead, SPF_item->getName(), nullptr,
                                                  "Unknown SPF Item type!",
                                                  ReturnCodes::PARSING_ERROR);
                        itemLength = -1;
                        return itemLength;
                }
    ...
    failureReport->addFailure(categoryNumber, index, itemToParse->getName(), nullptr,
                              "Unknown Explicit Length Item - only REF and SPF are permitted.",
                              ReturnCodes::PARSING_ERROR);
    return -1;
}
