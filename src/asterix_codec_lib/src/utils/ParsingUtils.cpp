    ...
    while (bytesRead < datablockLength) {
        ...
        for (unsigned short mandatoryFRN_toCheck : *(categoryDescription->getMandatoryItemFRNs())) {
            if (find(FRN_list.begin(), FRN_list.end(), mandatoryFRN_toCheck) == FRN_list.end()) {
                ...
                failureReport->addFailure(datablockCategory, bufferIndex + bytesRead, itemsDescription->at(mandatoryFRN_toCheck).getName(),
                                          entireDatablock,
        ...
        for (int i = 0; i < FRN_listSize; i++) {
            ...
            switch (itemType) {
                ...
                case ItemTypeEnum::RepetitiveLength:
                    itemLengthInByte = parseRepetitiveLengthItem(ASTERIX_datablock, itemToParse, bufferIndex + bytesRead,
                                                                 itemToParse->isItemToBeDecoded(),
                                                                 objectPool);
                    break;
                ...
                default: {
                    ...
                    memcpy(datablockToReport, (ASTERIX_datablock + bufferIndex), datablockLength);
                    failureReport->addFailure(datablockCategory, bufferIndex + bytesRead, itemToParse->getName(), datablockToReport,
                                              "Unknown Item type!",
...
int
ParsingUtils::parseFixedLengthItem(unsigned char *ASTERIX_datablock, Item *itemToParse, int index, bool isItemToBeDecoded, ParsedRecord *objectPool) {
...
int ParsingUtils::parseExtendedLengthItem(unsigned char *ASTERIX_datablock, Item *itemToParse, int index, bool isItemToBeDecoded,
...
int ParsingUtils::parseRepetitiveLengthItem(unsigned char *ASTERIX_datablock, Item *itemToParse, int index, bool isItemToBeDecoded,
                                            ParsedRecord *objectPool) {
    // Variables definition
        ...
        if (facadeCommonVariables->isRefDecodingEnabled() && refItemDescription->size() > 0) {
            ...
            for (unsigned int i = 0; i < itemsIndicator.size(); i++) {
                ...
                switch (REF_itemType) {
                    ...
                    case ItemTypeEnum::ExtendedLength:
                        bytesRead += parseExtendedLengthItem(ASTERIX_datablock, REF_item, index + bytesRead, REF_item->isItemToBeDecoded(),
                                                             objectPool);

                        break;

                    case ItemTypeEnum::RepetitiveLength:
                        bytesRead += parseRepetitiveLengthItem(ASTERIX_datablock, REF_item, index + bytesRead, REF_item->isItemToBeDecoded(),
                                                               objectPool);
                        break;

                    case ItemTypeEnum::CompoundLength:
                        bytesRead += parseCompoundLengthItem(ASTERIX_datablock, REF_item, index + bytesRead, REF_item->isItemToBeDecoded(),
                                                             subItemsDescription,
                                                             categoryNumber, objectPool);
                        break;

                    case ItemTypeEnum::ExplicitLength:
                        bytesRead += parseExplicitLengthItem(ASTERIX_datablock, REF_item, index + bytesRead, REF_item->isItemToBeDecoded(),
                                                             refItemDescription,
            ...
            for (unsigned int i = 0; i < elementsDescriptionSize; i++) {
                ...
                switch (SPF_itemType) {
                    case ItemTypeEnum::FixedLength:
                        ...
                        break;
                    case ItemTypeEnum::ExtendedLength:
                        bytesRead += parseExtendedLengthItem(ASTERIX_datablock, SPF_item, index + bytesRead, SPF_item->isItemToBeDecoded(),
                                                             objectPool);
                        break;
                    case ItemTypeEnum::CompoundLength:
                        bytesRead += parseCompoundLengthItem(ASTERIX_datablock, SPF_item, index + bytesRead, SPF_item->isItemToBeDecoded(),
                                                             subItemsDescription,
                                                             categoryNumber, objectPool);
                        break;
                    case ItemTypeEnum::RepetitiveLength:
                        bytesRead += parseRepetitiveLengthItem(ASTERIX_datablock, SPF_item, index + bytesRead, SPF_item->isItemToBeDecoded(),
                                                               objectPool);
                        break;
                    case ItemTypeEnum::ExplicitLength:
                        bytesRead += parseExplicitLengthItem(ASTERIX_datablock, SPF_item, index + bytesRead, SPF_item->isItemToBeDecoded(),
                                                             refItemDescription,
                                                             subItemsDescription, spfItemDescription, categoryNumber, objectPool);
                        break;
                    default:
