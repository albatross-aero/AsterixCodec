...
//        string message =
//                "Error while reading ASTERIX Configuration file: '" + astConfigurationFilePath + "', it is not a valid JSON.\n" + result.description()
//                + '\n';
...
//    }
    try {
        std::ifstream i(astConfigurationFilePath);
        json j;
        i >> j;
        currentAsterixConfiguration = j;
    ...
    if (commonVariables->isDebugEnabledForDecoder()) {
        ...
        for (auto kv : (*uapMap)) {
            cout << "Key: " << kv.first;
            cout << std::boolalpha << " - ToDecode: " << kv.second->isToBeDecoded() << " - ToEncode: " << kv.second->isToBeEncoded() << std::dec
                 << endl;
        }
    }
...
void JsonCategoriesDescriptionLoader::composeCategoriesDescriptionMapFromJsonFiles(AsterixConfiguration currentAsterixConfiguration,
                                                                                   string astConfigurationFilePath,
                                                                                   unordered_map<int, CategoryDescription *> *uapMap,
                                                                                   ReturnStatus *status) {
    /************************
     * Variable definitions *
    ...
    for (unsigned int i = 0; i < numberOfAsterixCategoriesSupported; i++) {
        ...
        if (categoryNumber <= 0) {
            ...
            message << "Category number is: " << categoryNumber << ". This filed must be > 0" << std::endl
        ...
        composeCategoryDescriptionFromJsonFiles(astConfigurationFilePath, currentAsterixConfiguration.getCategoriesConfiguration()[i],
        ...
        if (sizeOfDataSourcesUsingThisDescription > 0) {
            ...
            for (unsigned int j = 0; j < sizeOfDataSourcesUsingThisDescription; j++) {
                ...
                catAndSacSicInDecimal = JsonCategoriesDescriptionValidator::splitDatasourceString(
                        currentAsterixConfiguration.getCategoriesConfiguration()[i].getDataSourcesInDec()[j], categoryNumber,
                        astConfigurationFilePath, status);
...
void JsonCategoriesDescriptionLoader::composeCategoryDescriptionFromJsonFiles(string astConfigurationFilePath,
    ...
    CategoryItemsFilter categoryItemsFilters;
     ...
     *******************************************************************/
    performMarshallingFromExternalToInternalDescription(itemsDescription, reItemsDescription, spItemsDescription, uapAndMandatoryItems,
                                                        categoryDescription,
                                                        status);
    if (status->getCode() != ReturnCodes::SUCCESS) {
        return;
     ...
     ****************************************/
    JsonCategoriesDescriptionValidator::performValidationOfJsonFiles(itemsDescriptionPath, refDescriptionPath, spfDescriptionPath,
...
void JsonCategoriesDescriptionLoader::loadCategoriesFilter(string categoriesFilterPath, unordered_map<int, CategoryDescription *> *uapMap,
                                                           ReturnStatus *status) {
//    autojsoncxx::ParsingResult result;
...
//    }
    try {
        std::ifstream i(categoriesFilterPath);
        json j;
        i >> j;
        categoriesFilters = j;
    } catch (json::parse_error &e) {
...
//    }
    try {
        std::ifstream i(uapDescriptionPath);
        json j;
        i >> j;
...
//    }
    try {
        std::ifstream i(itemsDescriptionPath);
        json j;
        i >> j;
...
void JsonCategoriesDescriptionLoader::loadREForSPFDescription(string descriptionPath, ExternalCategoryDescription &itemsDescription,
                                                              ReturnStatus *status) {
    ifstream itemsJsonDescription;
//    autojsoncxx::ParsingResult result;
...
//    }
    try {
        std::ifstream i(descriptionPath);
        json j;
        i >> j;
        itemsDescription = j;
    } catch (json::parse_error &e) {
        string message =
                "Error while reading ASTERIX Configuration file: '" + descriptionPath + "', it is not a valid JSON.\n" +
                e.what() + "\n";
        status->setCode(ReturnCodes::AST_MALFORMED_DESCRIPTION);
        status->setMessage(message);
        return;
...
//    }
    try {
        std::ifstream i(filterPath);
        json j;
        i >> j;
        categoryItemsFilters = j;
    } catch (json::parse_error &e) {
        string message =
                "Error while reading ASTERIX Configuration file: '" + filterPath + "', it is not a valid JSON.\n" +
                e.what() + "\n";
        status->setCode(ReturnCodes::AST_MALFORMED_DESCRIPTION);
        status->setMessage(message);
        return;
...
void JsonCategoriesDescriptionLoader::putInOrderItemsDescription(string uapDescriptionPath, std::vector<std::string> uapList,
    ...
    for (unsigned int i = 0; i < uapList.size(); i++) {
        if (uapList.at(i) == "-") {
            ...
            string message =
                    "Error, " + uapList.at(i) + " definition not found in the 'itemsDescription' JSON file.\nError reading '" + uapDescriptionPath
                    + "' file.\n";
            status->setCode(ReturnCodes::AST_MALFORMED_DESCRIPTION);
            status->setMessage(message);
            return;
...
void JsonCategoriesDescriptionLoader::performMarshallingFromExternalToInternalDescription(ExternalCategoryDescription itemsDescription,
    ...
    if (refDescription.getItemCollection().size() > 0) {
        ...
        // Load COMPOUND SubITEMs description
        reCompoundSubItemCollectionTmp = marshallingToInnerDatamodel(&refDescription, LoadingTypeEnum::CompoundSubitemEnum, &mandatoryItemsList,
                                                                     status);
        if (status->getCode() != ReturnCodes::SUCCESS) {
            return;
        }
    ...
    if (spfDescription.getItemCollection().size() > 0) {
        ...
        // Load COMPOUND SubITEMs description
        spCompoundSubItemCollectionTmp = marshallingToInnerDatamodel(&spfDescription, LoadingTypeEnum::CompoundSubitemEnum, &mandatoryItemsList,
                                                                     status);
        if (status->getCode() != ReturnCodes::SUCCESS) {
            return;
        }
        ...
        for (unsigned int elementIndex = 0; elementIndex < elementAllDescription.size(); elementIndex++) {
            ...
            if (elementStringDescription.size() >= kMinimumSizeOfExtElementDescription) {
                ...
                message << "An incomplete Element description has been found for Item " << extItem.getName() << " ("
                        << kMinimumSizeOfExtElementDescription
...
void JsonCategoriesDescriptionLoader::checkAndSetFiltersForAllCategoriesDescription(string categoriesFilterPath, CategoriesFilters &categoriesFilters,
    ...
    FilterTypeEnum decoderFilterType;
    FilterTypeEnum encoderFilterType;
     ...
     */
    setFiltersForAllCategoriesDescription(true, decoderFilterType, encoderFilterType, decoderCategoryNumberToFilter, encoderCategoryNumberToFilter,
                                          uapMap, status);
    if (status->getCode() != ReturnCodes::SUCCESS) {
        return;
    }

    setFiltersForAllCategoriesDescription(false, decoderFilterType, encoderFilterType, decoderCategoryAndDatasourcesToFilter,
                                          encoderCategoryAndDatasourcesToFilter, uapMap, status);
    if (status->getCode() != ReturnCodes::SUCCESS) {
        return;
    }
}

void JsonCategoriesDescriptionLoader::setFiltersForAllCategoriesDescription(bool isLookingCatNumber, FilterTypeEnum decoderFilterType,
                                                                            FilterTypeEnum encoderFilterType,
                                                                            std::vector<int> &decoderCategoryNumberToFilter,
                                                                            std::vector<int> &encoderCategoryNumberToFilter,
    ...
    for (auto categoryDescription_it = uapMap->begin(); categoryDescription_it != uapMap->end(); categoryDescription_it++) {
         ...
         */
        if ((categoryFilterInDecimal_it = find(decoderCategoryNumberToFilter.begin(), decoderCategoryNumberToFilter.end(),
                                               currentCategoryDescriptionKey)) !=
            decoderCategoryNumberToFilter.end()) {    //Item exists in 'decoderFilterList'

            if (decoderFilterType == FilterTypeEnum::ExclusiveFilter)
         ...
         * If we are looking the Category Number + Datasource, we propagate the already setted value because that filter not involved other Category Number + Datasource
         */
        if ((categoryFilterInDecimal_it = find(encoderCategoryNumberToFilter.begin(), encoderCategoryNumberToFilter.end(),
                                               currentCategoryDescriptionKey)) !=
            encoderCategoryNumberToFilter.end()) {    //Item exists in 'decoderFilterList'

            if (encoderFilterType == FilterTypeEnum::ExclusiveFilter)
...
void JsonCategoriesDescriptionLoader::checkAndSetFiltersForCategoryDescription(string filterPath, CategoryItemsFilter categoryItemsFilters,
                                                                               CategoryDescription *categoryDescription, ReturnStatus *status) {
    /************************
     * Variable definitions *
     ************************/
    ...
    setFiltersForVectorOfItems(decoderFilterType, encoderFilterType, &decoderFilterList, &encoderFilterList, reItemDescription,
                               compoundSubItemsDescription);

    spItemDescription = categoryDescription->getSpfItemDescription();
    setFiltersForVectorOfItems(decoderFilterType, encoderFilterType, &decoderFilterList, &encoderFilterList, spItemDescription,
                               compoundSubItemsDescription);

    itemsDescription = categoryDescription->getItemCollection();
    setFiltersForVectorOfItems(decoderFilterType, encoderFilterType, &decoderFilterList, &encoderFilterList, itemsDescription,
                               compoundSubItemsDescription);
...
void JsonCategoriesDescriptionLoader::setFiltersForVectorOfItems(FilterTypeEnum decoderFilterType, FilterTypeEnum encoderFilterType,
                                                                 std::vector<std::string> *decoderFilterList,
                                                                 std::vector<std::string> *encoderFilterList, std::vector<Item> *itemsDescription,
    ...
    for (currentItem = itemsDescription->begin(); currentItem != itemsDescription->end(); currentItem++) {
        ...
        // - FilterTypeEnum == ExclusiveFilter -> EXCLUDE current Item from the decoding
        // - FilterTypeEnum == InclusiveFilter -> INCLUDE current Item in the decoding
        if ((itemToFilterForDecoder = find(decoderFilterList->begin(), decoderFilterList->end(), currentItemName)) !=
            decoderFilterList->end()) {    //Item exists in 'decoderFilterList'
        ...
        // - FilterTypeEnum == ExclusiveFilter -> EXCLUDE current item from the encoding
        // - FilterTypeEnum == InclusiveFilter -> INCLUDE current item in the encoding
        if ((itemToFilterForEncoder = find(encoderFilterList->begin(), encoderFilterList->end(), currentItemName)) !=
            encoderFilterList->end()) {    //Item exists in 'encoderFilterList'
