...
void ItemEncodingUtils::removeDecodedElementNameAndValueFromCollections(unsigned int indexOfElement,
...
unsigned int ItemEncodingUtils::calculateExtendedItemLength(vector<string *> *decodedElementNames, Item *itemToEncode,
    ...
    if (numberOfElementsNOTFound > 0 && numberOfElementsFound > 0) {
        string message = "One or more Elements for the Item " + itemToEncode->getName() +
                         " were not found in the input data, Item was skipped!";
...
vector<unsigned char> *ItemEncodingUtils::encodeFixedLengthItem(vector<string *> *decodedElementNames,
                                                                vector<ElementValue *> *decodedElementValues,
    ...
    for (unsigned int elementIndex = 0; elementIndex < numberOfItemElements; elementIndex++) {
        ...
        encodingElementLengthInByte =
                (encodingElementLengthInBit / 8) +
                1;    // +1 | an extra byte is required to manage the Element encoding phase - it will be scaled
        ...
        if (currentElementName != kSpare) {
            ...
            if (encodingElementValueIndex == -1) {
                if (numberOfElementsFound > 0) {
                    string message =
                            "One or more Elements for the Item " + itemToEncode->getName() +
                            " were not found in the input data, Item was skipped!";
                    encoderFailureReport->addFailure(categoryNumber, 0, itemToEncode->getName(), nullptr, message,
                                                     ReturnCodes::ENCODING_ERROR);
                    return nullptr;
            ...
            if (encodingElementValue == nullptr) {
                cerr << "Internal error, 'encodingElementValueIndex' is != -1 but the 'encodingElementValue' of "
                     << currentElementName
            ...
            switch (functionNameUsedForDecoding) {
                ...
                case FunctionNameEnum::toHexadecimal:
                    encodedElement = ElementEncodingUtils::encodeDecimal(encodingElementValue, currentValidRange,
                                                                         encodingElementLengthInBit,
                                                                         multiplicationFactorUsedForDecoding,
                                                                         &currentElementName, categoryNumber);
                ...
                case FunctionNameEnum::CA2toHexadecimal:
                    encodedElement = ElementEncodingUtils::encodeDecimalToCA2(encodingElementValue, currentValidRange,
                                                                              encodingElementLengthInBit,
                                                                              multiplicationFactorUsedForDecoding,
                                                                              &currentElementName,
                                                                              categoryNumber);
                ...
                case FunctionNameEnum::toASCII:
                    encodedElement = ElementEncodingUtils::encodeAscii(encodingElementValue,
                                                                       encodingElementLengthInBit);
                    break;
                case FunctionNameEnum::toTargetId:
                    encodedElement = ElementEncodingUtils::encodeTargetId(encodingElementValue,
                                                                          encodingElementLengthInBit);
                    break;
                case FunctionNameEnum::toMBData:
                    encodedElement = ElementEncodingUtils::encodeMBData(encodingElementValue,
                                                                        encodingElementLengthInBit);
                    break;
                default:
                    encoderFailureReport->addFailure(categoryNumber, 0, itemToEncode->getName(), nullptr,
                                                     "An unknown encoding function has been associated with '" +
                                                     currentElementName +
                                                     "' Element to encode!",
            ...
            if (encodedElement == nullptr) {
                string message =
                        "No raw value has been returned by the encoding function for the '" + currentElementName +
                        "' Element!";
                encoderFailureReport->addFailure(categoryNumber, 0, itemToEncode->getName(), nullptr, message,
            ...
            removeDecodedElementNameAndValueFromCollections(encodingElementValueIndex, decodedElementNames,
        ...
        if (encodingElementLengthInBit % 8 > 0) {
            remainingByte = encodedElement[bytesWithInformation -
                                           1];                        // Keep last byte of the element for the next loop
            remainingBitCounter = (bytesWithInformation * 8) -
                                  encodingElementLengthInBit;    // For sure the result is between 0 and 8
            remainingBitCounter = 8 -
                                  remainingBitCounter;    // Save the complement -> number of remaining free bits into 'remainingByte'
        }
    ...
    if (numberOfElementsNOTFound > 0) {
        string message =
                "One or more Elements for the current Item " + itemToEncode->getName() +
                " were not found in the input data, Item was skipped!";
        encoderFailureReport->addFailure(categoryNumber, 0, itemToEncode->getName(), nullptr, message,
                                         ReturnCodes::ENCODING_ERROR);
        return nullptr;
    ...
    if (commonVariables->isEncodingDebugEnabled()) {
        ...
        for (unsigned int i = 0; i < encodedItemBytestream->size(); i++) {
            cout << std::hex << std::setw(2) << std::setfill('0')
                 << static_cast<unsigned int>(encodedItemBytestream->at(i)) << " ";
        }
        cout << endl;
...
ItemEncodingUtils::encodeExtendedLengthItem(vector<string *> *decodedElementNames,
                                            vector<ElementValue *> *decodedElementValues,
                                            Item *itemToEncode, unsigned int categoryNumber) {
    ...
    for (unsigned int elementIndex = 0;
         elementIndex < numberOfElementsInItem && itemBitsStillToEncode > 0; elementIndex++) {
        ...
        if (encodingElementLengthInBit % 8 > 0) {
            ...
            remainingBitCounter = (bytesWithInformation * 8) -
                                  encodingElementLengthInBit;    // For sure the result is between 0 and 8
            remainingBitCounter = 8 -
                                  remainingBitCounter;    // Save the complement -> remaining number of free bits into 'remainingByte'
        }
...
vector<unsigned char> *
ItemEncodingUtils::encodeRepetitiveLengthItem(vector<string *> *decodedElementNames,
                                              vector<ElementValue *> *decodedElementValues,
                                              Item *itemToEncode, unsigned int categoryNumber) {
    ...
    do {
        // Scan each element of the current Item Description
        for (unsigned int elementIndex = 0;
             elementIndex < numberOfBaseElement; elementIndex++, indexInItemDescription++) {

            // Get the elementName
            if (numberOfRepetionInItemDescription >
                currentNumberOfRepetition) {    // element name is present in the element description
            ...
            if (currentElementName !=
                kSpare) {    //  no test elementName == "FX" -> in Repetitive item is not an allowed value
                ...
                switch (functionNameForDecoding) {
                    ...
                    case FunctionNameEnum::CA2toDecimal:
                    case FunctionNameEnum::CA2toHexadecimal:
                        encodedElement = ElementEncodingUtils::encodeDecimalToCA2(encodingElementValue,
                                                                                  currentValidRange,
                                                                                  encodingElementLengthInBit,
                                                                                  multiplicationFactorUsedForDecoding,
                                                                                  &currentElementName,
            ...
            if (encodingElementLengthInBit % 8 > 0) {
                remainingByte = encodedElement[bytesWithInformation -
                                               1];                    // Keeps last byte of the element for the next loop
                remainingBitCounter = (bytesWithInformation * 8) -
                                      encodingElementLengthInBit;    // For sure the result is between 0 and 8
                remainingBitCounter = 8 -
                                      remainingBitCounter;    // Saves the complement -> remaining number of free bits into 'remainingByte'
            }
...
vector<unsigned char> *
ItemEncodingUtils::encodeCompoundLengthItem(vector<string *> *decodedElementNames,
                                            vector<ElementValue *> *decodedElementValues,
                                            Item *itemToEncodeDescription, vector<Item> *compoundSubItemsDescription,
                                            unsigned int categoryNumber) {
    /************************
     * VARIABLES DEFINITION *
     ************************/
        ...
        if (currentSubItemName == kSpare) {
            ...
            switch (subitemType) {
                case ItemTypeEnum::FixedLength:
                    subitemBytestream = encodeFixedLengthItem(decodedElementNames, decodedElementValues, subItem,
                                                              categoryNumber);
                    break;

                case ItemTypeEnum::ExtendedLength:
                    subitemBytestream = encodeExtendedLengthItem(decodedElementNames, decodedElementValues, subItem,
                                                                 categoryNumber);
                    break;

                case ItemTypeEnum::RepetitiveLength:
                    subitemBytestream = encodeRepetitiveLengthItem(decodedElementNames, decodedElementValues, subItem,
                                                                   categoryNumber);
                    break;

                default:
                    encoderFailureReport->addFailure(categoryNumber, 0, currentSubItemName, nullptr,
                                                     "An unknown subitem type has been provided!",
                                                     ReturnCodes::ENCODING_ERROR);
                    break;
            }
    ...
    if (commonVariables->isEncodingDebugEnabled()) {
        cout << "\t FSPEC-compound:" << std::hex << std::setw(2) << std::setfill('0')
             << static_cast<unsigned int>(encodedItemBytestream->at(0))
             << endl;
        cout << "\t Item Length: " << std::dec << encodedItemBytestream->size() << endl;
    }
    return encodedItemBytestream;
...
vector<unsigned char> *
ItemEncodingUtils::encodeREFItem(vector<string *> *decodedElementNames, vector<ElementValue *> *decodedElementValues,
                                 Item *itemToEncode,
                                 vector<Item> *compoundSubItemsDescription, vector<Item> *refItemDescription,
                                 unsigned int categoryNumber) {
        ...
        if (subItem->isItemToBeEncoded()) {    // If the subItem is NOT to be encoded (setup coming from the JSON filters file) -> skip the encoding for that subItem
            ...
            switch (subitemType) {
                ...
                case ItemTypeEnum::CompoundLength:
                    subitemBytestream = encodeCompoundLengthItem(decodedElementNames, decodedElementValues, subItem,
                                                                 compoundSubItemsDescription,
                                                                 categoryNumber);
                    break;

                default:
                    encoderFailureReport->addFailure(categoryNumber, 0, subItem->getName(), nullptr,
                                                     "The given REF subitem type is unknown!",
                                                     ReturnCodes::ENCODING_ERROR);
                    break;
            }
        }
    ...
    encodedItemBytestream->insert(encodedItemBytestream->begin(),
                                  FSPEC);        // Append FSPEC at the beginning of the vector
...
vector<unsigned char> *
ItemEncodingUtils::encodeSPFItem(vector<string *> *decodedElementNames, vector<ElementValue *> *decodedElementValues,
                                 Item *itemToEncode,
                                 vector<Item> *compoundSubItemsDescription, vector<Item> *spfItemDescription,
                                 unsigned int categoryNumber) {
    ...
    for (unsigned int i = 0; i < numberOfElementsInItem; i++) {
        ...
        if (subItem->isItemToBeEncoded()) {    // If the subItem is NOT to be encoded (setup coming from the JSON filters file) -> skip the encoding for that subItem
            ...
            switch (subitemType) {
                ...
                default:
                    encoderFailureReport->addFailure(categoryNumber, 0, subItem->getName(), nullptr,
                                                     "The given SPF subitem type is unknown!",
                                                     ReturnCodes::ENCODING_ERROR);
                    break;
            }
...
vector<unsigned char> *
ItemEncodingUtils::encodeExplicitLengthItem(vector<string *> *decodedElementNames,
                                            vector<ElementValue *> *decodedElementValues,
                                            Item *itemToEncode, vector<Item> *compoundSubItemsDescription,
    ...
    if (itemName == kREF && facadeCommon->isRefEncodingEnabled()) {
        encodedItemBytestream = encodeREFItem(decodedElementNames, decodedElementValues, itemToEncode,
                                              compoundSubItemsDescription,
                                              refItemDescription,
                                              categoryNumber);
    } else if (itemName == kSPF && facadeCommon->isSpfEncodingEnabled()) {
        encodedItemBytestream = encodeSPFItem(decodedElementNames, decodedElementValues, itemToEncode,
                                              compoundSubItemsDescription,
                                              spfItemDescription,
                                              categoryNumber);
    }
