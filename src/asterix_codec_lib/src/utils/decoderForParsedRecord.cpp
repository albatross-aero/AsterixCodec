...
void decodeAllElementsOfRecord(vector<ParsedElement *> *allParsedElements, unsigned int parsedElementsSize,
    ...
    for (unsigned int k = 0; k < parsedElementsSize; k++) {
        ...
        if (itemDescription == nullptr || indexInItemVectors == -1) {
            decoderFailureReport->addFailure(categoryNumber, 0, "", nullptr,
                                             "Element description not found, Element skipped!",
        ...
        representationModeForDecoding = itemDescription->getRepresentationModeInEnumForDecodingAtIndex(
                indexInItemVectors);
        ...
        switch (functionNameToDecode) {
            case FunctionNameEnum::toDecimal:
                elementValue = DecodingUtils::toDecimal(parsedElementValue, &elementLengthInBit,
                                                        &multiplicationFactorToDecode,
                                                        &representationModeForDecoding,
                                                        valueRanges, categoryNumber, elementName,
                                                        decodedValueOPManager);
                break;
            case FunctionNameEnum::toHexadecimal:
                elementValue = DecodingUtils::toHexadecimal(parsedElementValue, &elementLengthInBit,
                                                            &multiplicationFactorToDecode,
                                                            &representationModeForDecoding, valueRanges, categoryNumber,
                                                            elementName,
                                                            decodedValueOPManager);
                break;

            case FunctionNameEnum::toOctal:
                elementValue = DecodingUtils::toOctal(parsedElementValue, &elementLengthInBit,
                                                      &multiplicationFactorToDecode,
                                                      &representationModeForDecoding,
            ...
            case FunctionNameEnum::CA2toDecimal:
                elementValue = DecodingUtils::CA2toDecimal(parsedElementValue, &elementLengthInBit,
                                                           &multiplicationFactorToDecode,
                                                           &representationModeForDecoding, valueRanges, categoryNumber,
                                                           elementName,
                                                           decodedValueOPManager);
            ...
            case FunctionNameEnum::CA2toHexadecimal:
                elementValue = DecodingUtils::CA2toHexadecimal(parsedElementValue, &elementLengthInBit,
                                                               &multiplicationFactorToDecode,
                                                               &representationModeForDecoding, valueRanges,
                                                               categoryNumber, elementName,
                                                               decodedValueOPManager);
                break;

            case FunctionNameEnum::toASCII:
                elementValue = DecodingUtils::toAscii(parsedElementValue, &elementLengthInBit,
                                                      &multiplicationFactorToDecode,
                                                      &representationModeForDecoding,
            ...
            case FunctionNameEnum::toTargetId:
                elementValue = DecodingUtils::toTargetId(parsedElementValue, &elementLengthInBit,
                                                         &multiplicationFactorToDecode,
                                                         &representationModeForDecoding,
                                                         categoryNumber, elementName, decodedValueOPManager);
                break;
            ...
            case FunctionNameEnum::toMBData:
                elementValue = DecodingUtils::toMBData(parsedElementValue, &elementLengthInBit,
                                                       &multiplicationFactorToDecode,
                                                       &representationModeForDecoding,
                                                       categoryNumber, elementName, decodedValueOPManager);
                break;
        ...
        if (debugEnabledForDecoder) {
            cout << "Decoded data: \t" << elementValue->getDecodedDataInString() << " .::. " << parsedElement->getName()
                 << endl;
        }
        if (dumpOnFileEnable) {
...
decodeParsedRecords(unsigned int numberOfConsumer, DecodedValueObjectPoolManager *decodedValueOPManager) {
    ...
    string outputFileName =
            common->getOutputTxtPathForDecoder() + "/out_decoding" + to_string(numberOfConsumer) + ".txt";
    bool dumpOnFileEnable = facadeCommon->isDumpOnFileEnableForDecoder();
    ofstream outputFile;
    ...
    while ((elementsOP = blockingQueuePtr->getObjectPoolForConsumer(currentThreadId)) != nullptr) {
        ...
        decodeAllElementsOfRecord(allParsedElements, sizeParsedElements, elementsOP->getRecordCategoryNumber(),
                                  decodedValueOPManager,
                                  decodedElementNames,
