...
string ParsedElement::getStringValue() {
        ...
        for (int j = 0; j < nBytes; j++) {
            if (parsedValue) {
                if (((j == nBytes - 1) || (j == 0)) && (valueLengthInBit % 8 != 0))
                    valueInString << std::hex << std::setw(1) << std::setfill('0')
                                  << static_cast<unsigned int>(parsedValue[j]) << "";
                else
                    valueInString << std::hex << std::setw(2) << std::setfill('0')
                                  << static_cast<unsigned int>(parsedValue[j]) << "";
            }
        }
