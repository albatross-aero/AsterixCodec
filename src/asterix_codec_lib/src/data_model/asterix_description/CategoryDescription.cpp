...
CategoryDescription::CategoryDescription(vector<Item> _itemsDescription, vector<Item> _compoundSubItemsDescription,
...
void CategoryDescription::convertInformationFromStringToEnum() {
    ...
    for (unsigned int i = 0; i < this->itemCollection.size(); i++) {
        if ((itemCollection.at(i).getItemType() != ItemTypeEnum::CompoundLength) &&
            (itemCollection.at(i).getItemType() != ItemTypeEnum::ExplicitLength)) {
...
void CategoryDescription::fillElementsMap() {
    ...
    // For each Item, scan all Elements and save the Element name and the pointer to its Item in the 'elementsMap'
    for (unsigned int i = 0; i < this->itemCollection.size(); i++) {
        item = &(itemCollection.at(i));
        if (item->getItemType() != ItemTypeEnum::CompoundLength &&
            item->getItemType() != ItemTypeEnum::ExplicitLength) {
