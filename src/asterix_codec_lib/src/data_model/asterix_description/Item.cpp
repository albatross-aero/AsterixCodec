...
unsigned short int Item::getElementsTotalSize() {
...
void Item::convertRepresentationModeForDecodingInStringToEnum() {
    ...
    if (size > 0) {
        for (unsigned int i = 0; i < size; i++) {
            representationModeForDecoding.push_back(
                    ConversionFromStringToEnum::getRepresentationModeEnumFromString(
                            representationModeForDecodingInString[i]));
            if (representationModeForDecoding[i] == RepresentationModeEnum::InvalidValue)
                cerr << "For '" << representationModeForDecodingInString[i]
                     << "' missing the corresponding RepresentationModeEnum." << endl;
        }
    }
}
...
void Item::convertFunctionNameForDecodingInStringToEnum() {
    ...
    if (size > 0) {
        for (unsigned int i = 0; i < size; i++) {
            functionNameForDecoding.push_back(
                    ConversionFromStringToEnum::getFunctionNameEnumFromString(functionNameForDecodingInString[i]));
            if (functionNameForDecoding[i] == FunctionNameEnum::InvalidValue)
                cerr << "For '" << functionNameForDecodingInString[i] << "' missing the corresponding FunctionNameEnum."
                     << endl;
        }
    }
}
...
void Item::addRepetitionInVectors(unsigned short startDimensionOfVector, unsigned short sizeOfFinalVector) {
    ...
    for (unsigned short int j = startDimensionOfVector; j < sizeOfFinalVector; j += numberOfElementsInTheModule) {
        ...
        suffix = string("_" + to_string(
                j / numberOfElementsInTheModule));    // j/numberOfElementsInTheModule -> number of repetition
