    ...
    while (bufferIndex < (*endOfBufferDecodingIndex)) {
        ...
        if ((bufferIndex + datablockLength) <= (*endOfBufferDecodingIndex)) {
            parsingResults.push_back(
                    parserTP->enqueue(ParsingUtils::parseDatablock, ASTERIX_datastream, datablockCategory, datablockLength, categoriesDescription,
                                      bufferIndex));
        } else {
            ...
            string message = "The Datablock provided starting at index " + to_string(bufferIndex) +
                             ...
                             + to_string(datablockLength) + " but the data stream ended at " + to_string(bufferIndex + datablockLength) +
                             " position.";
...
void DecoderInterface::printAllDecodedRecordsOnJson(RecordCollection *recordsToPrint) {
    ...
    for (unsigned int i = 0; i < recordsToPrintSize; i++) {
        ...
        for (unsigned int j = 0; j < sizeOfElementsToPrint; j++) {
            ...
            jsonElementValue.SetString(elementValue->getDecodedDataInString().c_str(), elementValue->getDecodedDataInString().length(),
                                       allocator);
