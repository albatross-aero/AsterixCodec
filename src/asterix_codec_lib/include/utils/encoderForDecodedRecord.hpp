...
encodeAllItemsFromRecordCollection(RecordCollection *decoderResult, unsigned int recordNumber, CategoryDescription *categoryDescription,
 ...
 */
vector<unsigned char> *
encodeAllItemsFromSingleRecord(Record *recordToEncode, CategoryDescription *categoryDescription, unsigned short int categoryNumber) {
    vector<unsigned char> *recordBytestream = new vector<unsigned char>();    // Raw Items inside a record
    vector<unsigned char> *itemBytestream = nullptr;
    ...
    for (unsigned int i = 0; i < numberOfItems && elementNames->size() > 0; i++) {
        ...
        if (itemToEncodeDescription->getName() != "-" && itemToEncodeDescription->isItemToBeEncoded()) {
            ...
            switch (itemToEncodeDescription->getItemType()) {
                ...
                case ItemTypeEnum::ExtendedLength:
                    itemBytestream = ItemEncodingUtils::encodeExtendedLengthItem(elementNames, elementValues, itemToEncodeDescription,
                                                                                 categoryNumber);
                    break;

                case ItemTypeEnum::RepetitiveLength:
                    itemBytestream = ItemEncodingUtils::encodeRepetitiveLengthItem(elementNames, elementValues, itemToEncodeDescription,
                                                                                   categoryNumber);
                    break;
