#include "DecoderBlockingQueue.h"
...
ParsedRecord* DecoderBlockingQueue::getObjectPoolFromQueue(unsigned int numberOfConsumer) {
	...
	if (!elementObjectPoolQueue.empty()) {
		...
		if (commonVariables->isDebugConsumerObjectPoolEnabled())
			cout << "[C" << numberOfConsumer << "] There is a free ElementObjectPool ready to be used" << endl;
		...
		if (commonVariables->isDebugConsumerObjectPoolEnabled())
			cout << "[C" << numberOfConsumer << "] Waiting - There is no ElementObjectPool ready to be used" << endl;
		...
		if (!elementObjectPoolQueue.empty()) {
			...
			if (commonVariables->isDebugConsumerObjectPoolEnabled())
				cout << "[C" << numberOfConsumer << "] Waking up - There is a free ElementObjectPool ready to be used" << endl;
