#include "BlockingQueueManager.h"
...
BlockingQueueManager::BlockingQueueManager() {
	...
	matchMap_threadIdProducerManager = std::map<unsigned long int, unsigned int>();
	matchMap_threadIdConsumerManager = std::map<unsigned long int, unsigned int>();
	...
	cout << endl << std::dec << "Elapsed time for Object pool initialization: " << chrono::duration_cast<chrono::milliseconds>(end - start).count() << " ms "
			<< endl;
}
...
ParsedRecord* BlockingQueueManager::getObjectPoolForProducer(unsigned long int threadId) {
	unsigned int objectPoolId = 0;
	auto poolIdIterator = matchMap_threadIdProducerManager.find(threadId);    // Check if the current thread already has an assigned queue already
	...
	if (poolIdIterator == matchMap_threadIdProducerManager.end()) {		// 'poolId' HAS NOT BEEN found..
		...
		matchMap_threadIdProducerManager.insert(std::pair<unsigned long int, unsigned int>(threadId, indexIntoMap));
...
void BlockingQueueManager::releaseObjectPoolForProducer(unsigned long int threadId, ParsedRecord* elementObjectPool) {
...
ParsedRecord* BlockingQueueManager::getObjectPoolForConsumer(unsigned long int threadId) {
	unsigned int poolId = 0;
	auto poolIdIterator = matchMap_threadIdConsumerManager.find(threadId);    // Check if the current thread already have a queue assigned to it
	...
	if (poolIdIterator == matchMap_threadIdConsumerManager.end()) {
		...
		unsigned int indexIntoMap = (matchMap_threadIdConsumerManager.size()) % (blockingQueuesForConsumer.size());    // It calculates the index to access the vector of queue modulo the number of queue
		matchMap_threadIdConsumerManager.insert(std::pair<unsigned long int, unsigned int>(threadId, indexIntoMap));
		poolId = indexIntoMap;
	} else {
