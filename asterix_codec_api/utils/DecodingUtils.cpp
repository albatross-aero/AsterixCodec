	...
	if (!currentElementRange->isEmpty) {    // Check if a validation range is set for the current Element
		...
		if (!currentElementRange->isMinIncluded) {
			if (!currentElementRange->isMaxIncluded) {    // Lower and upper bound NOT included
				...
				if (!((currentElementValue > currentElementRange->minValue) && (currentElementValue < currentElementRange->maxValue))) {
...
ElementValue* DecodingUtils::toDecimal(unsigned char *hexValue, int* elementLengthInBit, double* multiplicationFactorToDecode,
	...
	if (!isInsideRange) {
		...
		string message = "Element value '" + (*elementName) + "' out of range: it is " + to_string(returnValue) + ", but it was expected to be between "
				+ to_string(validRange->minValue) + " and " + to_string(validRange->maxValue) + ".";
		failureReport->addFailure(categoryNumber, 0, (*elementName), nullptr, message, ReturnCodes::OUT_OF_RANGE_DECODED_ELEMENT);
	}
	...
	switch (*representationMode) {
		...
		default:
			...
			string message = "The RepresentationMode given for the current Element '" + (*elementName) + "' is not permitted!";
			failureReport->addFailure(categoryNumber, 0, (*elementName), nullptr, message, ReturnCodes::UNKNOWN_REPRESENTATION_MODE);
			break;
	}
...
ElementValue* DecodingUtils::toHexadecimal(unsigned char *hexValue, int* elementLengthInBit, double* multiplicationFactorToDecode,
		RepresentationModeEnum* representationMode, Range* validRange, unsigned int categoryNumber, string* elementName,
		DecodedValueObjectPoolManager* decodedValueObjectPoolManager) {
	...
	switch (*representationMode) {
		case RepresentationModeEnum::Unsigned8bitIntegerValue:
			elementValue = decodedValueObjectPoolManager->getUnsignedInteger8bitValueFromOP();
			((UnsignedInteger8bitValue*) elementValue)->setDecodedValue(returnValue, RepresentationModeEnum::HexadecimalValue);
			break;

		case RepresentationModeEnum::Unsigned16bitIntegerValue:
			elementValue = decodedValueObjectPoolManager->getUnsignedInteger16bitValueFromOP();
			((UnsignedInteger16bitValue*) elementValue)->setDecodedValue(returnValue, RepresentationModeEnum::HexadecimalValue);
			break;

		case RepresentationModeEnum::Unsigned32bitIntegerValue:
			elementValue = decodedValueObjectPoolManager->getUnsignedInteger32bitValueFromOP();
			((UnsignedInteger32bitValue*) elementValue)->setDecodedValue(returnValue, RepresentationModeEnum::HexadecimalValue);
			break;

		case RepresentationModeEnum::Unsigned64bitIntegerValue:
			elementValue = decodedValueObjectPoolManager->getUnsignedInteger64bitValueFromOP();
			((UnsignedInteger64bitValue*) elementValue)->setDecodedValue(returnValue, RepresentationModeEnum::HexadecimalValue);
			break;

		case RepresentationModeEnum::Signed8bitIntegerValue:
			elementValue = decodedValueObjectPoolManager->getSignedInteger8bitValueFromOP();
			((SignedInteger8bitValue*) elementValue)->setDecodedValue(returnValue, RepresentationModeEnum::HexadecimalValue);
			break;

		case RepresentationModeEnum::Signed16bitIntegerValue:
			elementValue = decodedValueObjectPoolManager->getSignedInteger16bitValueFromOP();
			((SignedInteger16bitValue*) elementValue)->setDecodedValue(returnValue, RepresentationModeEnum::HexadecimalValue);
			break;

		case RepresentationModeEnum::Signed32bitIntegerValue:
			elementValue = decodedValueObjectPoolManager->getSignedInteger32bitValueFromOP();
			((SignedInteger32bitValue*) elementValue)->setDecodedValue(returnValue, RepresentationModeEnum::HexadecimalValue);
			break;

		case RepresentationModeEnum::Signed64bitIntegerValue:
			elementValue = decodedValueObjectPoolManager->getSignedInteger64bitValueFromOP();
			((SignedInteger64bitValue*) elementValue)->setDecodedValue(returnValue, RepresentationModeEnum::HexadecimalValue);
			break;
...
ElementValue* DecodingUtils::CA2toDecimal(unsigned char *hexValue, int* elementLengthInBit, double* multiplicationFactorToDecode,
		RepresentationModeEnum* representationMode, Range* validRange, unsigned int categoryNumber, string* elementName,
		DecodedValueObjectPoolManager* decodedValueObjectPoolManager) {
	ElementValue* elementValue = nullptr;
...
ElementValue* DecodingUtils::CA2toHexadecimal(unsigned char *hexValue, int* elementLengthInBit, double* multiplicationFactorToDecode,
		RepresentationModeEnum* representationMode, Range* validRange, unsigned int categoryNumber, string* elementName,
		DecodedValueObjectPoolManager* decodedValueObjectPoolManager) {
	ElementValue* elementValue = nullptr;
	int CA2version = 0, flippedVersion = 0;
...
ElementValue* DecodingUtils::toOctal(unsigned char *hexValue, int* elementLengthInBit, double* multiplicationFactorToDecode,
		RepresentationModeEnum* representationMode, Range* validRange, unsigned int categoryNumber, string* elementName,
		DecodedValueObjectPoolManager* decodedValueObjectPoolManager) {
	return toDecimal(hexValue, elementLengthInBit, multiplicationFactorToDecode, representationMode, validRange, categoryNumber, elementName,
						decodedValueObjectPoolManager);
}

ElementValue* DecodingUtils::toAscii(unsigned char *hexValue, int* elementLengthInBit, double* multiplicationFactorToDecode,
...
ElementValue* DecodingUtils::toTargetId(unsigned char *hexValue, int* elementLengthInBit, double* multiplicationFactorToDecode,
		RepresentationModeEnum* representationMode, unsigned int categoryNumber, string* elementName,
		DecodedValueObjectPoolManager* decodedValueObjectPoolManager) {
...
ElementValue* DecodingUtils::toMBData(unsigned char *hexValue, int* elementLengthInBit, double* multiplicationFactorToDecode,
		RepresentationModeEnum* representationMode, unsigned int categoryNumber, string* elementName,
		DecodedValueObjectPoolManager* decodedValueObjectPoolManager) {
