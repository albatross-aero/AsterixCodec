...
void decodeAllElementsOfRecord(vector<ParsedElement*>* allParsedElements, unsigned int parsedElementsSize, unsigned int categoryNumber,
		DecodedValueObjectPoolManager* decodedValueOPManager, vector<string*>* decodedElementNames, vector<ElementValue*>* decodedElementValues,
		ofstream* outputFile) {
	...
	for (unsigned int k = 0; k < parsedElementsSize; k++) {
		...
		if (itemDescription == nullptr || indexInItemVectors == -1) {
			decoderFailureReport->addFailure(categoryNumber, 0, "", nullptr, "Element description not found, Element skipped!", ReturnCodes::DECODING_ERROR);
		...
		switch (functionNameToDecode) {
			case FunctionNameEnum::toDecimal:
				elementValue = DecodingUtils::toDecimal(parsedElementValue, &elementLengthInBit, &multiplicationFactorToDecode, &representationModeForDecoding,
			...
			case FunctionNameEnum::toHexadecimal:
				elementValue = DecodingUtils::toHexadecimal(parsedElementValue, &elementLengthInBit, &multiplicationFactorToDecode,
															&representationModeForDecoding, valueRanges, categoryNumber, elementName, decodedValueOPManager);
				break;

			case FunctionNameEnum::toOctal:
				elementValue = DecodingUtils::toOctal(parsedElementValue, &elementLengthInBit, &multiplicationFactorToDecode, &representationModeForDecoding,
														valueRanges, categoryNumber, elementName, decodedValueOPManager);
				break;
			...
			case FunctionNameEnum::toASCII:
				elementValue = DecodingUtils::toAscii(parsedElementValue, &elementLengthInBit, &multiplicationFactorToDecode, &representationModeForDecoding,
														categoryNumber, elementName, decodedValueOPManager);
				break;

			case FunctionNameEnum::toTargetId:
				elementValue = DecodingUtils::toTargetId(parsedElementValue, &elementLengthInBit, &multiplicationFactorToDecode, &representationModeForDecoding,
															categoryNumber, elementName, decodedValueOPManager);
				break;

			case FunctionNameEnum::toMBData:
				elementValue = DecodingUtils::toMBData(parsedElementValue, &elementLengthInBit, &multiplicationFactorToDecode, &representationModeForDecoding,
														categoryNumber, elementName, decodedValueOPManager);
				break;
			...
			default:
				...
				decoderFailureReport->addFailure(categoryNumber, 0, (*elementName), nullptr,
													"For the given Element, an UNKNOWN decoding function name has been provided!", ReturnCodes::DECODING_ERROR);
				break;
		}
...
RecordCollection* decodeParsedRecords(unsigned int numberOfConsumer, DecodedValueObjectPoolManager *decodedValueOPManager) {
	 ...
	 * VARIABLES DEFINITION *
	 ************************/
	unsigned long int currentThreadId = 0;
	...
	vector<unsigned int>* collectionOfRecordCategoryNumber = new vector<unsigned int>();
	...
	while ((elementsOP = blockingQueuePtr->getObjectPoolForConsumer(currentThreadId)) != nullptr) {
		...
		decodeAllElementsOfRecord(allParsedElements, sizeParsedElements, elementsOP->getRecordCategoryNumber(), decodedValueOPManager, decodedElementNames,
