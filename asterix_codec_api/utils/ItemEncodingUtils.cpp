...
unsigned int ItemEncodingUtils::calculateExtendedItemLength(vector<string*>* decodedElementNames, Item* itemToEncode, unsigned int categoryNumber) {
	...
	return (finalCounterBitsNeeded / 8); 	// 'finalCounterBitsNeeded' MUST be exactly divisible by 8, because an Extended Length Item is built byte-by-byte
}
...
vector<unsigned char>* ItemEncodingUtils::encodeFixedLengthItem(vector<string*>* decodedElementNames, vector<ElementValue*>* decodedElementValues,
	...
	for (unsigned int elementIndex = 0; elementIndex < numberOfItemElements; elementIndex++) {
		...
		encodingElementLengthInByte = (encodingElementLengthInBit / 8) + 1;    // +1 | an extra byte is required to manage the Element encoding phase - it will be scaled
		...
		if (currentElementName != kSpare) {
			...
			if (encodingElementValue == nullptr) {
				cerr<< "Internal error, 'encodingElementValueIndex' is != -1 but the 'encodingElementValue' of " << currentElementName << " is not found in the input data!";
				return nullptr;
			}
			...
			if (encodedElement == nullptr) {
				...
				encoderFailureReport->addFailure(categoryNumber, 0, itemToEncode->getName(), nullptr, message, ReturnCodes::UNKNOWN_REPRESENTATION_MODE);
				return nullptr;
			}
	...
	if(numberOfElementsNOTFound > 0){
		string message = "One or more Elements for the current Item " +itemToEncode->getName() +" were not found in the input data, Item was skipped!";
		encoderFailureReport->addFailure(categoryNumber, 0, itemToEncode->getName(), nullptr, message, ReturnCodes::ENCODING_ERROR);
		return nullptr;
	}
...
vector<unsigned char>* ItemEncodingUtils::encodeExtendedLengthItem(vector<string*>* decodedElementNames, vector<ElementValue*>* decodedElementValues,
		Item* itemToEncode, unsigned int categoryNumber) {
	...
	for (unsigned int elementIndex = 0; elementIndex < numberOfElementsInItem && itemBitsStillToEncode > 0; elementIndex++) {
		...
		encodingElementLengthInByte = (encodingElementLengthInBit / 8) + 1;    // +1 just because the encoding phase of an element allocates one byte more the normal size
		if (encodingElementLengthInBit % 8 > 0)
			encodingElementLengthInByte++;
		...
		if (currentElementName != kSpare && currentElementName != kFX) {
			...
			if (encodingElementValueIndex == -1) {
				cerr<< "Internal error, this point should be not reachable if 'calculateExtendedItemLength' calculate Item Length equal to 0. Error for Item " << currentElementName << ".";
				return nullptr;
			}
...
vector<unsigned char>* ItemEncodingUtils::encodeRepetitiveLengthItem(vector<string*>* decodedElementNames, vector<ElementValue*>* decodedElementValues,
		Item* itemToEncode, unsigned int categoryNumber) {
...
vector<unsigned char>* ItemEncodingUtils::encodeCompoundLengthItem(vector<string*>* decodedElementNames, vector<ElementValue*>* decodedElementValues,
	...
	if (commonVariables->isEncodingDebugEnabled()) {
		cout << "\t FSPEC-compound:" << std::hex << std::setw(2) << std::setfill('0') << static_cast<unsigned int>(encodedItemBytestream->at(0)) << endl;
		cout << "\t Item Length: " << std::dec << encodedItemBytestream->size() << endl;
	}
	return encodedItemBytestream;
}
...
vector<unsigned char>* ItemEncodingUtils::encodeREFItem(vector<string*>* decodedElementNames, vector<ElementValue*>* decodedElementValues, Item* itemToEncode,
...
vector<unsigned char>* ItemEncodingUtils::encodeSPFItem(vector<string*>* decodedElementNames, vector<ElementValue*>* decodedElementValues, Item* itemToEncode,
...
vector<unsigned char>* ItemEncodingUtils::encodeExplicitLengthItem(vector<string*>* decodedElementNames, vector<ElementValue*>* decodedElementValues,
		Item* itemToEncode, vector<Item> *compoundSubItemsDescription, vector<Item> *refItemDescription, vector<Item> *spfItemDescription,
	...
	if (itemName == kREF && facadeCommon->isRefEncodingEnabled()) {
		encodedItemBytestream = encodeREFItem(decodedElementNames, decodedElementValues, itemToEncode, compoundSubItemsDescription, refItemDescription,
				categoryNumber);
	} else if (itemName == kSPF && facadeCommon->isSpfEncodingEnabled()) {
		encodedItemBytestream = encodeSPFItem(decodedElementNames, decodedElementValues, itemToEncode, compoundSubItemsDescription, spfItemDescription,
				categoryNumber);
	}
