...
void CodecInterface::loadAsterixConfiguration(std::string pathToAsterixConfigurationFile, ReturnStatus &status) {
	...
	if (pathToAsterixConfigurationFile.find(".dat") != std::string::npos) {
		JsonCategoriesDescriptionLoader::getCategoriesDescriptionMapFromSerialization(pathToAsterixConfigurationFile, categoriesDescription, &status);
	} else {
		JsonCategoriesDescriptionLoader::getCategoriesDescriptionMapFromJSONFile(pathToAsterixConfigurationFile, categoriesDescription, &status);
	}
}

void CodecInterface::serializeAsterixConfiguration(std::string pathToArchiveFile, ReturnStatus &status) {
	status.clear();
	JsonCategoriesDescriptionLoader::serializeCategoriesDescriptionMap(pathToArchiveFile, common->getCategoriesDescription(), &status);
}
...
void CodecInterface::setUpEnvironment(ReturnStatus &status) {
	...
	sizeDecodedValueOP = (facadeCommon->getMaximumNumberOfDatablock() * facadeCommon->getAverageNumberOfRecordPerDatablock())
			/ facadeCommon->getNumberOfDecodingThreads();
	if ((facadeCommon->getMaximumNumberOfDatablock() * facadeCommon->getAverageNumberOfRecordPerDatablock()) % facadeCommon->getNumberOfDecodingThreads() != 0)
		sizeDecodedValueOP++;
...
void cleanDirectory(const char* directoryPath) {
	...
	while ((entry = readdir(directory)) != NULL) {
		...
		if (strcmp(entry->d_name, ".") && strcmp(entry->d_name, "..")) {
			snprintf(filePathToDelete, (size_t) PATH_MAX, "%s/%s", directoryPath, entry->d_name);    // create path of file to remove
