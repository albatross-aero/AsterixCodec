...
TEST_F(CodecTest, LoadsAsterixBytestream) {
	...
	ASSERT_EQ(ASTERIX_datastream[0], 0x15) << "Error while checking the datablock CAT number!";						// Checks CAT No
	ASSERT_EQ(ASTERIX_datastream[1], 0x01) << "Error while checking the datablock LEN most significant byte!";// Checks LEN MSByte
	ASSERT_EQ(ASTERIX_datastream[2], 0x4f) << "Error while checking the datablock LEN least significant byte!";// Checks LEN LSByte
	//
	ASSERT_EQ(ASTERIX_datastream[ASTERIX_inputFileSize - 2], 0xb0) << "Error while checking the second to last byte of the bytestream!";// Checks second to last Byte
	ASSERT_EQ(ASTERIX_datastream[ASTERIX_inputFileSize - 1], 0x38) << "Error while checking the very last byte of the bytestream";// Checks the very last Byte
}
...
TEST_F(CodecTest, LoadsCategoryJSONDescription) {
	JsonCategoriesDescriptionLoader::getCategoriesDescriptionMapFromJSONFile(ASTERIX_categoryRepositoryPath, &categoriesDescription, &status);
...
TEST_F(CodecTest, DatablockCategoryIdentification) {
	vector<unsigned char> ASTERIX_StdCivilMilitaryAppCategories = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x11, 0x12,
			0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1f, 0x20, 0x21, 0x22, 0x30, 0x3d, 0x3e, 0x3f, 0x41 };
	vector<unsigned char> ASTERIX_SpecialAndMilitaryAppCategories = { 0x80, 0x96, 0x97, 0x98, 0x99, 0xcc, 0xdd, 0xef, 0xf0 };
	vector<unsigned char> ASTERIX_NATO_ACCSAppCategories = { 0x9b, 0x9c, 0x9d, 0x9e, 0x9f, 0xa0, 0xb4, 0xc9, 0xcb };
	vector<unsigned char> ASTERIX_NoStdCivilMilitaryAppCategories = { 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xf9, 0xfa, 0xfb, 0xfc, 0xfd, 0xfe, 0xff };

	vector<unsigned char> ASTERIX_Categories;
	ASTERIX_Categories.reserve(
			ASTERIX_StdCivilMilitaryAppCategories.size() + ASTERIX_SpecialAndMilitaryAppCategories.size() + ASTERIX_NATO_ACCSAppCategories.size()
...
//	EXPECT_EQ(ASTERIX_NoStdCivilMilitaryAppCategories.size(), 15);
	...
	ASTERIX_Categories.insert(ASTERIX_Categories.end(), ASTERIX_StdCivilMilitaryAppCategories.begin(), ASTERIX_StdCivilMilitaryAppCategories.end());
	ASTERIX_Categories.insert(ASTERIX_Categories.end(), ASTERIX_SpecialAndMilitaryAppCategories.begin(), ASTERIX_SpecialAndMilitaryAppCategories.end());
	ASTERIX_Categories.insert(ASTERIX_Categories.end(), ASTERIX_NATO_ACCSAppCategories.begin(), ASTERIX_NATO_ACCSAppCategories.end());
	ASTERIX_Categories.insert(ASTERIX_Categories.end(), ASTERIX_NoStdCivilMilitaryAppCategories.begin(), ASTERIX_NoStdCivilMilitaryAppCategories.end());

	EXPECT_EQ(ASTERIX_Categories.size(), 64);

	ASSERT_NE(std::find(ASTERIX_Categories.begin(), ASTERIX_Categories.end(), ASTERIX_datastream[0]), ASTERIX_Categories.end())<< "An unknown datablock Category value has been read from the bytestream!";
...
TEST_F(CodecTest, LoadsAsterixCategoriesDescription) {
	...
	bytesRead += 1;
	datablockLength = (short int) (ASTERIX_datastream[bufferIndex + 2] + ((ASTERIX_datastream[bufferIndex + 1] << 8) & 0xFF00));
	bytesRead += 2;
	...
	ASSERT_EQ(categoryDescription_it, categoriesDescription.end())<< "SAC=32 SIC=50 has been found on \"asterixConfiguration.json!\", even if it shouldn't be there.";
...
TEST_F(CodecTest, ParsesFixedLengthItem) {
	...
	categoriesDescriptionLoadingAndFSPECreation(&bufferIndex, &bytesRead, (unsigned short int) 21, &categoryDescription, &itemsDescription, &FRN_list);
	...
	bytesRead += ParsingUtils::parseFixedLengthItem(ASTERIX_datastream, itemToParse, bufferIndex + bytesRead, true, parsedElementsOP);
...
TEST_F(CodecTest, ParsesExtendedLengthItem) {
	...
	bytesRead += ParsingUtils::parseExtendedLengthItem(ASTERIX_datastream, itemToParse, bufferIndex + bytesRead, true, parsedElementsOP);
...
TEST_F(CodecTest, ParsesRepetitiveLengthItem) {
	...
	bytesRead += ParsingUtils::parseRepetitiveLengthItem(ASTERIX_datastream, itemToParse, bufferIndex + bytesRead, true, parsedElementsOP);
...
TEST_F(CodecTest, ParsesCompoundItem) {
	...
	bytesRead += ParsingUtils::parseCompoundLengthItem(ASTERIX_datastream, itemToParse, bufferIndex + bytesRead, true, compoundSubItemsDescription,
...
TEST_F(CodecTest, ParsesRefItem) {
	...
	bytesRead += ParsingUtils::parseExplicitLengthItem(ASTERIX_datastream, itemToParse, bufferIndex + bytesRead, true, refItemDescription,
														compoundSubItemsDescription, spfItemDescription, (unsigned int) 21, parsedElementsOP);
...
TEST_F(CodecTest, ParsesSpfItem) {
	...
	categoriesDescriptionLoadingAndFSPECreation(&bufferIndex, &bytesRead, (unsigned short int) 62, &categoryDescription, &itemsDescription, &FRN_list);
	...
	bytesRead += ParsingUtils::parseExplicitLengthItem(ASTERIX_datastream, itemToParse, bufferIndex + bytesRead, true, refItemDescription,
														compoundSubItemsDescription, spfItemDescription, (unsigned int) 62, parsedElementsOP);
