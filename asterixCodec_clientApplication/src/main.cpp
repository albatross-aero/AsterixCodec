 ...
 */
int readCommandLineArgs(int argc, char *argv[], std::string &fileName, string &ASTERIX_categoryRepositoryPath, string &justSerialiseAndQuit,
	...
	try {
		...
		option_description.add_options()("help", "produce help message")("file_path,f", po::value<string>(), "To specify ASTERIX byte stream file")(
				"datablock", po::value<unsigned int>(), "To set the number of maximum number of datablock to parse/decode")(
						"record", po::value<unsigned int>(), "To set the average number of record per datablock")("thread", po::value<unsigned short int>(),
								"To set the total number of threads to be used")(
										"consumer", po::value<unsigned short int>(), "To set the number of threads to be used from the consumer")(
												"producer", po::value<unsigned short int>(), "To set the number of threads to be used from the producer")(
														"encoder", po::value<unsigned short int>(), "To set the number of threads to be used from the encoder")(
																"encoder_off", "To disable the encoding function. [ENABLED by default]")("encode", "To run the Encoding Client")(
																		"debug,d", "To enable the output printing on the console for the DECODER. [DISABLED by default]")(
																				"debug_encoder,c", "To enable the output printing on the console for the ENCODER. [DISABLED by default]")(
																						"re,r", "To enable RE item parsing. [RE JUMPED by default]")("spf", "To enable SPF item parsing. [SPF JUMPED by default]")(
																								"filter", "To enable FILTERING. [FILTER disable by default]")("pool", po::value<unsigned int>(), "To set the number of object pool to be used")(
...
void printPerformanceInformation(unsigned long long timing, bool isDecoderTiming, unsigned long long recordNumber, unsigned long ASTinput_fileSize) {
	double parsingRate = 0.0;

	if (isDecoderTiming) {
		cout << endl << std::dec << "** DECODER ** " << endl << "\tElapsed time: " << timing << " ms || " << timing / 1000 << " s .::. " << recordNumber
				<< " records decoded" << endl;
	} else {
		cout << endl << std::dec << "** ENCODER ** " << endl << "\tElapsed time: " << timing << " ms || " << timing / 1000 << " s .::. " << recordNumber
	...
	if (timing != 0) {
		...
		cerr << "Error while calculating the parsing rate, expressed in records per second or in megabyte per second." << endl;
	}
}
...
void decodeAndEncodeAsterixFile(string ASTERIX_inputFilePath, bool encodingFunctionOn, ReturnStatus &status) {
	...
	// decodingDatablocksFromFile decode blocks of N datablocks of ASTERIX_datastream - the index in the ASTERIX_datastream is managed by the library
	// If decodingDatablocksFromFile return nullptr -> no more datablocks to decode
	while ((decodedRecords = decoderInterface->decodingDatablocksFromFile()) != nullptr) {
...
void decodeAndEncodeFromOtherSource(bool encodingFunctionOn) {
	...
	unsigned long datablockLength = 311;
	unsigned int currentEncodedRecordNumber = 0;

	unsigned char ASTERIX_datablock[] = {  };
...
int main(int argc, char *argv[]) {
	 ...
	 **********************************/
	cmdLineArgsParsing_rc = readCommandLineArgs(argc, argv, ASTERIX_inputFilePath, ASTERIX_categoryRepositoryPath, justSerialiseAndQuit, encodingFunctionOn);
